 ...
 * (RFC-0025 Phase 5 / AISDLC-270 / AC#1).
 *
 ...
 *   - MTTR per subclass (first capture → fix done date)
 *   - Recurrence rate (fixed bugs that recurred within 30 days)
...
function renderTable(report: QualityCorpusReport): string {
  ...
  if (metrics.recurrence.length > 0) {
    lines.push(`Recurrence Rate (within ${30} days of fix — OQ-3 default window)`);
    lines.push(`-------------------------------------------------------------------`);
      ...
      (y) =>
        y
          ...
          .option('recurrence-window-days', {
            ...
            default: 30,
            describe:
              'Recurrence-detection window in days (OQ-3 default: 30). A fixed bug that sees a new capture of the same subclass within this window counts as a recurrence.',
          })
          .option('format', {
            type: 'string',
